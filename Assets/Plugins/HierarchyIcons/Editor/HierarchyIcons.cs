...
namespace HierarchyIcons.Editor
{
    ...
    class HierarchyIcons
    {
        ...
        public static void Repaint(Object obj, Rect selectionRect)
        {
            if (CreateSingleton())
            {
                if (obj != null)
                {
                    ...
                    {
                        RepaintComponent(gameObject, selectionRect, iconPos);
                    }
        ...
        public static void RepaintComponent(GameObject go, Rect selectionRect, Rect iconPos)
        {
            for (int i = 0; i < list.icons.Count; i++)
            {
                ...
                {
                    // Set icon size to match default Unity icon size
                    int offset = -18;
                    int sideLength = 18;
                    iconPos.size = new Vector2(sideLength, sideLength);

                    // Place the icon on the left side, exactly where the default Unity icon appears
                    iconPos.position = new Vector2(selectionRect.xMin + offset, selectionRect.yMin);

                    // Get the hierarchy background color
                    // Color backgroundColor = EditorStyles.label.normal.background.GetPixel(0, 0);
                    Color backgroundColor = new Color(0.219f, 0.219f, 0.219f);

                    // Draw a rectangle with the background color to hide the default icon
                    EditorGUI.DrawRect(new Rect(selectionRect.xMin + offset, selectionRect.yMin, sideLength, sideLength), backgroundColor);
                    ...
                    {
                        var icon = iconsNames[key];
                        GUI.Label(iconPos, icon);
                    }
